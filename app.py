...
def validate_key(api_key):
    ...
    now = datetime.utcnow()
    if now > key_entry.created_at + timedelta(days=key_entry.expiration_days):
        return False, "Key expired"
    if now.date() > key_entry.last_reset.date():
...
def handle_requests():
    ...
    if not api_key:
        ...
        active_keys = [k for k in keys if datetime.utcnow() < k.created_at + timedelta(days=k.expiration_days)]
...
def admin_dashboard():
    ...
    keys = APIKey.query.all()
    total_keys = len(keys)
    active_keys = len([k for k in keys if datetime.utcnow() < k.created_at + timedelta(days=k.expiration_days)])
    total_requests = sum([k.total_requests for k in keys])
    expired_keys = len([k for k in keys if datetime.utcnow() >= k.created_at + timedelta(days=k.expiration_days)])
    
    keys_data = []
    for key in keys:
        expires_at = key.created_at + timedelta(days=key.expiration_days)
...
def purchase_key():
    keys = APIKey.query.all()
    total_requests = sum([k.total_requests for k in keys])
    return render_template('purchase_new.html', total_requests=total_requests)
